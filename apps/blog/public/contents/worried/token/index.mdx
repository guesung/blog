...
웹만 고려했을 경우, 이전 프로젝트에서는 보안을 고려하여 Refresh Token의 경우 쿠키에 저장하여 XSS공격으로부터 보호하였고, Access Token의 경우 localStorage에서 관리하여 CSRF 공격으로부터 보호하였다.

그런데 이번 프로젝트는 모바일 앱에서 웹뷰를 띄우는 형태로 진행이 된다. 이 경우에도 Refresh Token과 Access Token을 모두 사용하여 토큰을 관리하여야 할까? 그에 앞서서, 다른 프로젝트는 어떻게 했을까?

## 토큰 관리 리서치

### 0. [React Native 웹뷰 토큰 관리 - 코드 분석](https://guesung.notion.site/React-Native-c3a2c54ab7ff4a56b73a1112e737afe9)

### 1. 트레바리 : 쿠키에 토큰 하나만 저장한다.

웹뷰로 구현한 [트레바리 사이트](https://m.trevari.co.kr)를 먼저 탐색해보았다. 
...
두 번째로, 최근에 PC도 웹뷰로 통일한 [무신사 사이트](https://www.musinsa.com)를 살펴보았다.
...
1. 임시성: 브라우저 세션이 끝날 때(즉, 브라우저를 닫을 때) 자동으로 삭제됩니다.
2. 만료 시간 없음: 명시적인 만료 시간(Expiration time)이 설정되어 있지 않습니다.
3. 메모리 저장: 대부분의 경우 브라우저 메모리에 저장되며, 디스크에 기록되지 않습니다.
4. 보안: 브라우저를 닫으면 삭제되므로, 영구 쿠키에 비해 상대적으로 더 안전합니다.
5. 사용 목적: 주로 로그인 세션 유지, 장바구니 정보 임시 저장 등에 사용됩니다.
</Callout>
...
[디프만의 아맞다](https://github.com/depromeet/ahmatda-web/tree/main)에서는 토큰을 localStorage에 저장하였다. Access Token과 Refresh Token을 따로 구분하지 않았다.

대신에, 사용자를 구분하는 userToken과 firebase 알림 토큰인 fcmToken을 또 사용하였다.

### 4. wordbe(티스토리 블로그) : asyncStorage
...
<Callout title="치명적인 단점" initialOpen type='minus'>
  위와 같은 방식으로 localStorage를 이용하면 Server에서 먼저 데이터를 받아와서 페이지를 구성해서 넘겨주는 SSR 방식을 활용하지 못한다.
</Callout>


위와같은 Next.js의 SSR, SSG같은 장점을 활용하지 못한다는 치명적인 단점이 존재한다. server에서 먼저 데이터를 fetching한다는 것은 server에서 토큰에 접근할 수 있어야 한다는 것을 의미한다. server에 데이터를 저장하고, 접근할 수 있는 방법으로는 쿠키가 있다. 

사실 이전 프로젝트를 리팩토링할 때 위와 비슷한 고민을 하였고, CSR -> SSR로 마이그레이션 하는 과정에서 동일한 이유로 저장 위치를 기존 로컬 스토리지에서 쿠키로 변경한 적이 있다.
...
4. **리서치 결과**:
   - 트레바리와 무신사는 쿠키에 토큰을 저장했습니다.
   - 디프만 - 아맞다와 디프만 - 영감탱은 로컬 스토리지에 토큰을 저장했습니다.
   - Wordbe는 `asyncStorage`를 활용했습니다.
